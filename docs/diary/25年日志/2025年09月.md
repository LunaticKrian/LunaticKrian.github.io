...
继续学习 Docker：Docker 网络（三种默认网络模式：bridge、host、none）

> PS：Docker 使用、基础概念学习完成，后续开始学习 Kubernetes 相关的组件，以支撑对云原生和DevOps的使用和理解

---

## 2025年09月28日

- 【个人身体】🔴重要：最近一段时间有种强烈感觉，随着年纪不断增加，加上高强度的工作，身体开始有点走下坡路，所以决定开始保养好身体。
  - 坚持锻炼：有氧慢跑
  - 忌口：忌口零食、辛辣、冷冻，喝酒、宵夜（睡前-22点后吃东西）等等；
  - 规律作息：最晚睡觉时间24点，最佳理想休息时间23点；

- 【技术学习】开始 Kubernetes 阶段的学习，主要是理解 Kubernetes 各个组件的核心概念以及对DevOps的配置
（目前公司正在搭建这么一套新的构建部署环境，抓紧时间学习）

> PS：Kubernetes 基础组件学习完成之后，开始学习 Golang

- 【编程语言】Python：目前进度：10%（目前对基础的代码编写有了一个初步的掌握，还有些语法待学习）
    - 理解 `__init__.py` 在项目中的应用
    - 理解 `import xxx` 和 `from xxx import xxx` 的区别

- 【个人能力】英语：目前进度：0% （计划从背单词开始，计划覆盖考研的单词和部分雅思的单词：10000个）

> PS：考研目标6000个（进度60%达成，明年2026年），整个英语单词的背诵计划是10000个（进度100%达成日期不定）
...
- 【FlowGram.AI插件化构建流程引擎】https://flowgram.ai/index.html
...
- 《操作系统：设计与实现》：了解操作系统基本概念
- 《Linux内核设计与实现》：了解Linux的运行机制
- 《深入理解Linux内核》：具体研究Linux内核源码
- 《深入理解LINUX网络技术内幕》：Linux网络部分学习
