...
```golang
var sliceDef []type = make([]type, len)
```
...
copy 函数的行为：只复制 slice 中的元素到另一个 slice；

- 对于值类型（int、float、struct 等），复制的是值；
- 对于引用类型（slice、map、指针等），复制的是引用。
...
动态类型（即concrete type，也叫具体类型）是程序运行时系统才能看见的类型。
